...
import model.{ AlreadyRegistered, GameSession, NoGame, TypedSocketCommand }
...
object AddPlayerCommand extends TypedSocketCommand[AddPlayer]( "addPlayer", AddPlayer.form ) {
  ...
  override def executeTyped( sessionController:SessionController, sessionID:String, gameSession:Option[GameSession], data:AddPlayer ):Try[JsValue] = gameSession match {
    case Some( gameSession ) =>
      if( gameSession.players.get( sessionID ).use( o => o.isDefined && o.get.isEmpty ) ) {
        controllerAction( gameSession, _.action( _.addPlayer( data.color, data.name ) ) )
